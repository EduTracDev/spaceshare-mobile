...
type Props = {
  ...
  onSelectedChange: (selected: { [key: string]: number }) => void;
};

export default function AddOnSection({ addOns, onTotalChange, selected, onSelectedChange }: Props) {
  ...
  return (
    <View style={styles.section}>
      ...
      >
        ...
        <Feather name={expanded ? 'chevron-up' : 'chevron-down'} size={18} color="#6A7181" />
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  section: { borderTopWidth: 1, borderTopColor: '#F2F4F7', paddingTop: 4 },
  header: {
    flexDirection: 'row', alignItems: 'center',
    justifyContent: 'space-between', paddingVertical: 14,
  },
  title: { fontFamily: 'MonaSans-Bold', fontSize: 16, color: '#020203', letterSpacing: -0.3 },
  box: { backgroundColor: '#F3F4F6', borderRadius: 12, padding: 14, gap: 14, marginBottom: 14 },
  item: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  itemLeft: { gap: 2, flex: 1 },
  itemName: { fontFamily: 'Inter-Regular', fontWeight: '600', fontSize: 14, color: '#020203' },
  itemSub: { fontFamily: 'Inter-Regular', fontSize: 12, color: '#6A7181' },
  ...
  counterText: {
    fontFamily: 'Inter-Regular', fontWeight: '600', fontSize: 14,
    color: '#020203', minWidth: 16, textAlign: 'center',
  },
});
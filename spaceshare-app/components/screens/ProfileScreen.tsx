...
const SETTINGS_TEMPLATE: Omit<MenuItem, 'route'>[] = [
  { label: 'Raise a Dispute', icon: 'alert-octagon', color: '#F97316' },
  { label: 'Bank Account', icon: 'credit-card', color: '#8B5CF6' },
  { label: 'Notifications', icon: 'bell', color: '#EF4444' },
  { label: 'Change Password', icon: 'lock', color: '#F59E0B' },
  { label: 'Contact Us', icon: 'headphones', color: '#06B6D4' },
];
const SETTINGS_SLUGS = ['dispute', 'bank-account', 'notifications', 'change-password', 'contact'];

const RESOURCES_TEMPLATE: Omit<MenuItem, 'route'>[] = [
  { label: 'Term of Use', icon: 'book-open', color: '#F97316' },
  { label: 'Privacy Policy', icon: 'shield', color: '#8B5CF6' },
];
...
export default function ProfileScreen({ basePath, loginRoute, isHost = false }: ProfileScreenProps) {
  ...
  const SETTINGS: MenuItem[] = SETTINGS_TEMPLATE.map((item, i) => ({
    ...item,
    route: `${basePath}/${SETTINGS_SLUGS[i]}`,
  }));
  const RESOURCES: MenuItem[] = RESOURCES_TEMPLATE.map((item, i) => ({
    ...item,
    route: `${basePath}/${RESOURCES_SLUGS[i]}`,
  }));
    ...
    <View style={s.root}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        ...
        <View style={s.avatarRow}>
          ...
          <TouchableOpacity style={s.editBtn} onPress={() => router.push(`${basePath}/edit-profile` as any)}>
